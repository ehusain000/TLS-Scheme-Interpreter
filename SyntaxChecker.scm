...
;(simple-check 'car)
;(simple-check 'cdr)
(simple-check '(lambda (x) (* x x)))

;to check for the presence of unbound error
;we are to design and develope a helper function
;half-ribs that only contains the left half each entry
...
(define table1 '(((x y z) (1 2 3)) ((x y z) (1 2 3)) ((x y z) (1 2 3))))
;(half-ribs table1)
;((x y z) (x y z) (x y z))


(define (check-unbound x table)
  (if
   (lookup-in-table x  table initial-table)
   #t
   #f))

(check-unbound 'x table1)
